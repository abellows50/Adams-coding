...
def check_db(filename):
    for line in open(filename, "r"):
        ...
        name = parts[0]
        try:
            fantastical = float(parts[1])
            historical = float(parts[2])
            content = float(parts[3])
            story = float(parts[4])
        except ValueError:
            return (False, line)
        for rating in [fantastical, historical, content, story]:
...
def query_for(filename, name=None, fantastical=None, historical=None, content=None, story=None):
    ...
    for line in open(filename, "r"):
        ...
        other = Book.Book(name, fantastical, historical, content, story)

        if name is not None and other.name != name:
            continue
        if fantastical is not None and other.fantastical != fantastical:
            continue
        if historical is not None and other.historical != historical:
            continue
        if content is not None and other.content != content:
            continue
        if story is not None and other.story != story:
            continue
